...
<body>
    ...
    <header>
        <div class="logo">
            <img src="https://epu.edu.iq/wp-content/uploads/2025/01/EPU-Logo-n2nnn7.png" alt="EPU Logo" class="epu-logo" id="homeLogoBtn">
            <h1>
    ...
    <div class="container" id="mainPage">
        <section class="search-section">
            <div class="search-box">
                ...
                <div class="search-suggestions" id="searchSuggestions" style="position: absolute; top: 100%; right: 0; left: 0; background: white; border: 1px solid var(--gray-color); border-radius: 8px; margin-top: 5px; max-height: 250px; overflow-y: auto; display: none; z-index: 1000; box-shadow: var(--shadow);"></div>
            </div>
            ...
            <div style="background: white; padding: 1.5rem; border-radius: 12px; box-shadow: var(--shadow);">
                <form id="studentRegistrationForm">
                    ...
                    <div class="form-group">
                        ...
                        <input type="tel" id="regPhoneNumber" placeholder="07501234567" required>
                    </div>
        ...
        <div id="bookManagementSection">
            ...
            <div class="table-header">
                <div class="table-search-box">
                    ...
                    <i class="fas fa-search"></i>
                    <div class="search-suggestions" id="adminSearchSuggestions" style="position: absolute; top: 100%; right: 0; left: 0; background: white; border: 1px solid var(--gray-color); border-radius: 8px; margin-top: 5px; max-height: 250px; overflow-y: auto; display: none; z-index: 1000; box-shadow: var(--shadow);"></div>
                </div>
            </div>
        ...
        <div id="registeredStudentsSection" style="display: none;">
            ...
            <div class="table-header">
                <div class="table-search-box">
                    ...
                    <i class="fas fa-search"></i>
                    <div class="search-suggestions" id="studentSearchSuggestions" style="position: absolute; top: 100%; right: 0; left: 0; background: white; border: 1px solid var(--gray-color); border-radius: 8px; margin-top: 5px; max-height: 250px; overflow-y: auto; display: none; z-index: 1000; box-shadow: var(--shadow);"></div>
                </div>
            </div>
                ...
                <table class="admin-table">
                    <thead>
                        <tr>
                            ...
                            <th>مۆبایل</th>
                            <th>کتێب</th>
                            <!-- بەپێی ڕێکخستنەکە ستوونی بەروارەکان دیاری دەکرێن لە جاڤاسکریپتدا -->
                            <th id="dateColumnHeader1">بەروار</th>
                            <th id="dateColumnHeader2"></th>
                            <th id="dateColumnHeader3"></th>
                            <th>بارودۆخ</th>
                            <th>کردارەکان</th>
                        </tr>
                    </thead>
    ...
    <footer>
        <div class="footer-content">
            ...
            <p><i class="fas fa-phone"></i> 07503983598</p>
    ...
    <script type="module">
        ...
        const dateColumnHeader1 = document.getElementById('dateColumnHeader1');
        const dateColumnHeader2 = document.getElementById('dateColumnHeader2');
        const dateColumnHeader3 = document.getElementById('dateColumnHeader3');
        ...
        function setupEventListeners() {
            ...
            document.addEventListener('click', (e) => {
                ...
                if (!adminSearchSuggestions.contains(e.target) && e.target !== adminSearchInput) {
                    adminSearchSuggestions.style.display = 'none';
                }
                if (!studentSearchSuggestions.contains(e.target) && e.target !== studentSearchInput) {
                    studentSearchSuggestions.style.display = 'none';
                }
            ...
            adminSearchInput.addEventListener('input', (e) => {
                ...
                if (searchTerm.trim() !== '') {
                    showAdminSearchSuggestions(searchTerm, adminSearchSuggestions);
                } else {
                    adminSearchSuggestions.style.display = 'none';
                }
            ...
            // کلیک لە ڕسمەکە بۆ گەڕانەوە بۆ پەڕەی سەرەکی
        ...
        function updateStatistics() {
            ...
            const pendingRequestsCount = allRequests.filter(r => !r.pasand).length;
            const borrowedBooksCount = approvedRequests.length;
            
            document.getElementById('totalBooks').textContent = totalBooks;
            document.getElementById('wishlistTotal').textContent = pendingRequestsCount;
            document.getElementById('borrowedBooks').textContent = borrowedBooksCount;
            
            navWishlistCount.textContent = wishlist.length || 0;
            navBorrowedCount.textContent = borrowedBooksCount || 0;
            
            document.getElementById('adminTotalBooks').textContent = totalBooks;
            document.getElementById('adminPendingRequests').textContent = pendingRequestsCount;
            ...
            myBorrowedBooks.forEach(request => {
                ...
                borrowedItem.innerHTML = `
                    ...
                    <div class="borrowed-item-info">
                        ...
                        ${request.approvedDate ? `<div class="borrowed-item-date">بەرواری وەرگیراو: ${new Date(request.approvedDate).toLocaleDateString('ku')}</div>` : ''}
        ...
        async function displayRegisteredStudentsFromFirebase(searchTerm = '') {
            registeredStudentsTable.innerHTML = '<tr><td colspan="10" style="text-align: center; padding: 2rem;"><div class="loading-spinner"></div></td></tr>';
            ...
            try {
                ...
                // *** ڕێکخستنی ستوونەکانی بەروار بەپێی view ***
                // 1. بەشی چاوەڕوان (Pending): تەنها بەرواری داواکردن
                if (currentStudentView === 'pending') {
                    dateColumnHeader1.textContent = 'بەرواری داواکردن';
                    dateColumnHeader2.style.display = 'none';
                    dateColumnHeader3.style.display = 'none';
                }
                // 2. بەشی وەرگیراوەکان (Approved): تەنها بەرواری وەرگیراو
                else if (currentStudentView === 'approved') {
                    dateColumnHeader1.textContent = 'بەرواری وەرگیراو';
                    dateColumnHeader2.style.display = 'none';
                    dateColumnHeader3.style.display = 'none';
                }
                // 3. بەشی گەڕێندراوەکان (Returned): هەر سێ بەروارەکە
                else if (currentStudentView === 'returned') {
                    dateColumnHeader1.textContent = 'بەرواری داواکردن';
                    dateColumnHeader2.textContent = 'بەرواری وەرگیراو';
                    dateColumnHeader3.textContent = 'بەرواری گەڕێندراو';
                    dateColumnHeader2.style.display = 'table-cell';
                    dateColumnHeader3.style.display = 'table-cell';
                }
        ...
        function displayRegisteredStudentsTable(requests) {
            ...
            requests.forEach(request => {
                ...
                const requestDate = request.requestDate ? new Date(request.requestDate).toLocaleDateString('ku') : 'نەزانراو';
                const approvedDate = request.approvedDate ? new Date(request.approvedDate).toLocaleDateString('ku') : '---';
                const returnDate = request.returnDate ? new Date(request.returnDate).toLocaleDateString('ku') : '---';
                
                const row = document.createElement('tr');
                
                // بەشی چاوەڕوان: تەنها بەرواری داواکردن
                if (currentStudentView === 'pending') {
                    row.innerHTML = `
                ...
                // بەشی وەرگیراوەکان: تەنها بەرواری وەرگیراو
                else if (currentStudentView === 'approved') {
                    row.innerHTML = `
                        <td><strong>${request.name || '---'}</strong></td>
                        <td>${request.department || '---'}</td>
                ...
                // بەشی گەڕێندراوەکان: هەر سێ بەروارەکە
                else if (currentStudentView === 'returned') {
                    row.innerHTML = `
                        <td><strong>${request.name || '---'}</strong></td>
                        <td>${request.department || '---'}</td>
            ...
            requests.forEach(request => {
                ...
                if (currentStudentView === 'pending') {
                    mobileRow.innerHTML = `
                        ...
                        <div class="mobile-table-cell">
                            <span class="mobile-table-label">بەرواری داواکردن:</span>
                            <span class="mobile-table-value">${requestDate}</span>
                        </div>
                        <div class="mobile-table-cell">
                            <span class="mobile-table-label">بارودۆخ:</span>
                            <span class="mobile-table-value"><span class="status-badge ${statusClass}">${statusText}</span></span>
                ...
                // بەشی وەرگیراوەکان: تەنها بەرواری وەرگیراو
                    ...
                    mobileRow.innerHTML = `
                        ...
                        <div class="mobile-table-cell">
                            <span class="mobile-table-label">بەرواری وەرگیراو:</span>
                            <span class="mobile-table-value">${approvedDate}</span>
                        </div>
                        <div class="mobile-table-cell">
                            <span class="mobile-table-label">بارودۆخ:</span>
                            <span class="mobile-table-value"><span class="status-badge ${statusClass}">${statusText}</span></span>
                ...
                // بەشی گەڕێندراوەکان: هەر سێ بەروارەکە
                    ...
                    mobileRow.innerHTML = `
                        ...
                        <div class="mobile-table-cell">
                            <span class="mobile-table-label">بەرواری گەڕێندراو:</span>
                            <span class="mobile-table-value">${returnDate}</span>
                        </div>
                        <div class="mobile-table-cell">
                            <span class="mobile-table-label">بارودۆخ:</span>
                            <span class="mobile-table-value"><span class="status-badge ${statusClass}">${statusText}</span></span>
        ...
        function showSearchSuggestions(searchTerm, suggestionsContainer) {
            ...
            if (filteredBooks.length === 0) {
                suggestionsContainer.innerHTML = '<div class="suggestion-item" style="padding: 10px; cursor: default; color: #666;">هیچ کتێبێک نەدۆزرایەوە</div>';
            } else {
                filteredBooks.slice(0, 5).forEach(book => {
                    ...
                    suggestion.innerHTML = `
                        ...
                        <small style="color: #666;">${book.author || 'نەناسراو'} - ${book.zhanar}</small>
                    `;
        ...
        function showAdminSearchSuggestions(searchTerm, suggestionsContainer) {
            const filteredBooks = books.filter(book => 
                book.naw.toLowerCase().includes(searchTerm) ||
                (book.author && book.author.toLowerCase().includes(searchTerm)) ||
                (book.zhanar && book.zhanar.toLowerCase().includes(searchTerm))
            ...
            if (filteredBooks.length === 0) {
                ...
                filteredBooks.slice(0, 5).forEach(book => {
                    ...
                    suggestion.addEventListener('click', () => {
                        adminSearchInput.value = book.naw;
                        suggestionsContainer.style.display = 'none';
                        displayAdminBooks(book.naw.toLowerCase());
                    });
                    suggestionsContainer.appendChild(suggestion);
                });
            }
        ...
        function showStudentSearchSuggestions(searchTerm) {
            ...
            if (filteredRequests.length === 0) {
                studentSearchSuggestions.innerHTML = '<div class="suggestion-item" style="padding: 10px; cursor: default; color: #666;">هیچ خوێندکارێک نەدۆزرایەوە</div>';
            } else {
                filteredRequests.slice(0, 5).forEach(request => {
                    ...
                    suggestion.innerHTML = `
                        ...
                        <small style="color: #666;">${request.department} - ${request.bookTitle}</small>
                    `;
                    suggestion.addEventListener('mouseover', () => {
                        suggestion.style.backgroundColor = '#f5f5f5';
                    });
            ...
            studentSearchSuggestions.style.display = 'block';
        }
